...
//Lehrer Name für iOS kürzen wegen der Email
if (!is_null($_GET["swift"]) && strlen($_GET["Lehrer"]) > 4) {
  $_GET["Lehrer"] = substr($_GET["Lehrer"],0,3);
}
if (!isset($_GET["Lehrer"])) {
  die("Kein Lehrer gewählt.");
}elseif ($_GET["Lehrer"] == "help") {
...
$_GET["Lehrer"] = strtoupper($_GET["Lehrer"]);
error_reporting(0);
  ...
  if (mysqli_num_rows($result) > 0) {
      ...
      while($row = mysqli_fetch_assoc($result)) {
        if ($fname == $row["Kuerzel"]) {
          if (isset($_GET["swift"])) {
            $fname = $row["Name"]. " %28". $row["Kuerzel"] . "%29";
            return $fname;
          }else {
...
if ($_GET["Woche"]== "n") {
  $output = "In der nächsten Woche hast du den Lehrer in diesen Stunden:\n\n";
}elseif ($_GET["Woche"]== "diese") {
  $output = "In dieser Woche hast du den Lehrer in diesen Stunden:\n\n";
}
...
if (isset($event['DTSTART'])) {
  # code...
  if (strpos($event['DESCRIPTION'], $_GET["Lehrer"]) !== false) {
    ...
    if (isset($_GET["swift"])) {
      $getWochenuebersicht = "fach=".$event['SUMMARY']."&";
        ...
        $getWochenuebersicht .= "lehrer=" . urlencode(lehrerName($event['DESCRIPTION'])) . "&"; // Ausgabe: Lehrer  = HIN, HNG, ...
...
if (isset($_GET["swift"])) {
  ...
  if ($o == 0) {
    if ($_GET["Woche"] == "n") {
    echo file_get_contents("$URL_SWIFTWOUE?keinunterricht=Kein%20Unterricht%20mit%20dem%20Lehrer%20in%20der%20nächsten%20Woche!");
    }elseif ($_GET["Woche"] == "diese") {
      echo file_get_contents("$URL_SWIFTWOUE?keinunterricht=Kein%20Unterricht%20mit%20dem%20Lehrer%20in%20dieser%20Woche!");
    }

  }

} else {
  if ($output == "In dieser Woche hast du den Lehrer in diesen Stunden:\n\n") {
    $output = "In dieser Woche hast du keinen Unterricht mit diesem Lehrer." ;
  }elseif ($output == "In der nächsten Woche hast du den Lehrer in diesen Stunden:\n\n") {
    $output = "In der nächsten Woche hast du keinen Unterricht mit diesem Lehrer." ;
    }
    echo $output;
}}
/*
